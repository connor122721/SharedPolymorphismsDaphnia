...
# eSMC Output
setwd('/project/berglandlab/connor/eSMC_output_11_14_24_gen5/')
...
# All files
files <- rbindlist(lapply(list.files(pattern = "rds"), readRDS), fill = T) %>%
  right_join(fin %>% select(Sample, Species, Continent, cont))

# Output esmc
esmc <- na.omit(data.table(files %>%
                     group_by(Sample, chromosome) %>%
                     mutate(time_index = seq_along(x)) %>%
                     group_by(time_index, cont) %>%
                     summarize(x1=mean(x, na.rm = T),
                               meany1=mean(y, na.rm = T),
                               uciy1=quantile(y, na.rm = T, probs = 0.95),
                               lciy1=quantile(y, na.rm = T, probs = 0.05)) %>%
                     select(cont, x1, meany1, uciy1, lciy1),
                   analysis="eSMC"))

# Get distribution for beta and sigma
beta <- data.table(na.omit(files[!cont=="Daphnia.pulexcaria.NorthAmerica"]) %>%
  group_by(cont, Sample) %>%
  summarize(b=mean(beta)))

#saveRDS(beta, file = "../data/beta_eSMC_gen5.rds")
#saveRDS(esmc, file = "../data/eSMC_gen5.rds")

# Summary stats of beta
confint(lm(b~1, data=beta[cont=="Daphnia.pulex.NorthAmerica"]), level = 0.95)
mean(beta[cont=="Daphnia.pulex.NorthAmerica"]$b)

confint(lm(b~1, data=beta[cont=="Daphnia.pulex.Europe"]), level = 0.95)
mean(beta[cont=="Daphnia.pulex.Europe"]$b)
...
# Output plot
pdf("/project/berglandlab/connor/SuppFig3.new_10gens.pdf", width=8, height=6, useDingbats = T)
plot
dev.off()
