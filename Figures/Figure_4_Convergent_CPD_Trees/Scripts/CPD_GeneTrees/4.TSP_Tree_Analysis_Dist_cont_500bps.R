...
# Trees
files.tree <- system("ls -f -R /project/berglandlab/connor/candgene/tree_exon_tsp_250/*treefile", intern = T)

# Exon metadata
exon <- data.table(rbind(data.table(read.csv("candgene/exons.genome.list.tspset5", header = F)),
                         data.table(read.csv("candgene/exons.genome.list.tspset6", header = F))))
colnames(exon) <- c("ch", "gene", "classified", "simpleAnnot")
...
#ds.tree2 <- data.table(readRDS("/project/berglandlab/connor/candgene/phylo.dist.250bptsp.nohaplo.med.rds"))
