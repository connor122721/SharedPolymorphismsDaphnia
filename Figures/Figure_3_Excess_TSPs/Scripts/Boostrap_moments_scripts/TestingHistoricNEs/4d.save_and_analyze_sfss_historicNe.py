...
csv_dir = dir + "sfss/sfs_csvs_rescale80/"
...
# Output file
output_file = dir + "output_eSMC/sfs_eSMC_rescaled80_statistics" + "." + modeli + "." + repli + ".txt"
...
def get_shared_allele_prop(sfs, maf_threshold=0.05):
...
with open(output_file, "a") as fout:
    ...
    for ns in [[20, 20], [100, 100]]:
        ...
        if modeli == "split_mig":
            ...
            save_sfs_as_csv(sfs_split_mig_model, f"sfs_split_mig_model_{str(ns[0])}.{repli}")
        ...
        # Conditional run "Split + No Mig" model
            ...
            save_sfs_as_csv(sfs_split_no_mig_model, f"sfs_split_no_mig_model_{str(ns[0])}.{repli}")
