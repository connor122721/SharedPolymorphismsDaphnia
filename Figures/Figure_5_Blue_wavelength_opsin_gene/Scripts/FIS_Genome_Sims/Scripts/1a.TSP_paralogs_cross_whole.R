...
# Read in total SNPs
tot_snps <- data.table(fread("/scratch/csm6hg/data/snps_new"))
#tot_snps = tot_snps[gene %in% c("Daphnia11806-RA")]
...
classify_het_parents <- function(classi) {
  ...
  # Parent 1
  SC_1 <- unique(fin[SC %in% SC1][WildSequenced==1][Nonindependent==0]$Sample)
  
  # Parent 1
  SC_2 <- unique(fin[SC %in% SC2][WildSequenced==1][Nonindependent==0]$Sample)
