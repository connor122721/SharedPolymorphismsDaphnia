...
vcf.fun <- function(lower.maf, upper.maf) {
    ...
    pp <- foreach(chr.num = 1:dim(chrom.meta)[1], .combine="rbind", .errorhandling = "remove") %do% {
      ...
      genome_syn <- diversity.stats.between(genome_syn, keep.site.info = T)
      genome_syn <- diversity.stats(genome_syn)
      genome_syn <- F_ST.stats(genome_syn, detail = T)
      
      genofile1 <- set.populations(genofile, 
                                  list("pulex_euro_d8"=c(fin[cont=="Daphnia.pulex.Europe"][pondID=="D8"]$Sample),
                                       "pulex_euro_dbunk"=c(fin[cont=="Daphnia.pulex.Europe"][pondID=="DBunk"]$Sample),
                                       "pulex_euro_dcat"=c(fin[cont=="Daphnia.pulex.Europe"][pondID=="DCat"]$Sample),
                                       "pulex_euro_pond17"=c(fin[cont=="Daphnia.pulex.Europe"][pondID=="Pond17"]$Sample),
                                       "pulex_euro_d10"=c(fin[cont=="Daphnia.pulex.Europe"][pondID=="D10"]$Sample)))
      
      genofile1 <-  F_ST.stats(genofile1, detail = T)
    
      test <- data.table(position=as.integer(names(unlist(genofile1@region.stats@site.FST))),
                 FST=unlist(genofile1@region.stats@site.FST),
                 chrom=unique(chrom.meta$chrom)[chr.num])
      
      test[!is.na(FST)]
      
      # Summarize
      dt <- data.table(fread("/project/berglandlab/connor/new_vcf2/out.weir.fst"))
      colnames(dt) <- c("chrom", "position", "FST")
      
      test2 <- data.table(dt[position %in% tot[chrom=="Scaffold_1863_HRSCAF_2081"]$position] %>% 
        left_join(tot, by=c("position", "chrom")) %>% 
        filter(!is.na(FST)) %>%
        mutate(classified=case_when(classified=="shared_poly"~"TSP",
                                    TRUE~"Not TSP")) %>% 
        group_by(classified, simpleAnnot) %>% 
